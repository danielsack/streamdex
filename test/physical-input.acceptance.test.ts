...
describe("physical input acceptance", () => {
  ...
  it("advertises the deliberate context touch controls", () => {
    const manifest = JSON.parse(source("io.streamdex.plugin.sdPlugin/manifest.json"));
    const action = manifest.Actions.find((a: { UUID: string }) => a.UUID.endsWith(".workdesk"));
    expect(action.Encoder.TriggerDescription.Touch).toContain("labelled");
    expect(action.Encoder.TriggerDescription.LongTouch).toContain("Hold");
  });
