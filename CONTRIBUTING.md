...
GitHub Actions runs on an isolated macOS Apple Silicon runner with read-only repository permissions. It checks the committed source and payload before rebuilding, uses pinned dependencies and action commits, and runs the same local check command. Forked PR workflows require approval before execution. CI does not upload build artifacts, install on real hardware, publish releases, or deploy the demo. Logs are public, so use fictional data even in failure messages. A successful CI run does not establish hardware compatibility or approve a release.
