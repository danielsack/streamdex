...
// Reject unsupported commands before touching Accessibility or a running app.
let runtimeActions = Set(["dictation-start", "dictation-stop", "target-capture", "composer-read", "target-verify", "target-check", "dispatch", "new-project", "workflow", "route", "mode-read", "mode-toggle", "workdesk-global", "workdesk-controls", "read", "model", "reasoning"])
guard runtimeActions.contains(action) else {
    emit(ControlResult(ok: false, action: action, requested: nil, model: nil,
                       effort: nil, reasonCode: "UNAVAILABLE",
                       message: "Unsupported command."), exitCode: 1)
}
...
do {
    ...
    case "workdesk-global":
        guard let requested else { throw ControlError.failed("Missing work desk command.") }
        try workdeskGlobal(requested, app: app)
        emit(ControlResult(ok: true, action: action, requested: requested, model: nil, effort: nil, message: "Dispatched the app command."), exitCode: 0)
    case "workdesk-controls":
        let labels = captureAXSnapshot(appElement).elements.filter {
            $0.role == "AXButton" && !$0.hidden &&
            ([$0.title, $0.description, $0.help].joined(separator: " ").lowercased().contains("voice"))
        }.map { [$0.title, $0.description, $0.help].joined(separator: " | ") }
        emit(ControlResult(ok: true, action: action, requested: nil, model: nil, effort: nil, message: labels.joined(separator: "\n")), exitCode: 0)
    case "read":
    ...
    case "reasoning":
        ...
        let label = selection.label
        var transaction = try preflight()
        let originalModel = try readPickerState(transaction.window).0
        if let expected = selection.expectedModel {
            guard let original = originalModel else { throw ControlError.failed("No model witness", "UNAVAILABLE") }
            let label = normalized(original)
            guard normalized(expected) == label || normalized(expected).hasSuffix("-" + label) else {
                throw ControlError.failed("Model changed before reasoning apply", "TARGET_MISMATCH")
            }
        }
        dismissOpenMenus()
        let operationSnapshot = captureAXSnapshot(transaction.window)
        ...
        guard state.0 == originalModel else {
            throw ControlError.failed("Model changed while applying reasoning", "TARGET_MISMATCH")
        }
        try recordTransactionOperation(&transaction)
        try postflightTargetTransaction(
            &transaction,
    ...
    default:
        throw ControlError.failed(
            "Usage: codex-ui-control target-capture | dictation-start WITNESS | dictation-stop | composer-read | read | model SLUG | reasoning LEVEL | mode-read MODE | mode-toggle MODE | new-project"
        )
    }
} catch {
