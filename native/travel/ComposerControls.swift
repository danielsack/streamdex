...
func approvalMode(from value: String) -> String? {
    ...
    if text.contains("fullaccess") { return "full-access" }
...
func dispatchControl(
    ...
    switch payload {
    case "workdesk-voice":
        let endLabels = Set(["End voice chat", "End call", "Stop voice chat"])
        let startLabels = Set(["Start new voice chat", "Start voice chat"])
        func voiceButtons(_ snapshot: AXSnapshot, labels: Set<String>) -> [ElementInfo] {
            snapshot.elements.filter {
                $0.role == "AXButton" && $0.enabled && !$0.hidden &&
                labels.contains($0.description.isEmpty ? $0.title : $0.description)
            }
        }
        let endings = voiceButtons(initial, labels: endLabels)
        let candidates = endings.isEmpty ? voiceButtons(initial, labels: startLabels) : endings
        guard candidates.count == 1, let button = candidates.first else {
            throw ControlError.failed("No unique visible voice-chat control is available.", "UNAVAILABLE")
        }
        try pressAccessibilityControl(button.element)
        guard waitUntil(timeout: 3.5, operation: { () -> Bool? in
            let next = captureAXSnapshot(appElement, maximumDepth: maximumCodexWindowTraversalDepth)
            return voiceButtons(next, labels: endings.isEmpty ? startLabels : endLabels).isEmpty ? true : nil
        }) != nil else {
            throw ControlError.failed("Voice control did not visibly change.", "UNCHANGED")
        }
    case "accept", "approve":
