...
function isProposedPlan(event: RolloutEvent): boolean {
  ...
  return p.content.some(
    (part) =>
      ...
      /^\s*<proposed_plan>[\s\S]+<\/proposed_plan>\s*$/.test(part.text),
  );
}
...
export function reduceRolloutEvents(
  ...
  for (const event of events) {
    ...
    if (event.type === "event_msg" && COMPLETE_EVENT_TYPES.has(subtype)) {
      ...
      status = proposedPlan
        ? "needs-input"
        : completedAt > (options.acknowledgedAt ?? 0)
          ? "unread"
          : "read";
      detail = proposedPlan
        ? "Plan ready: continue or give feedback"
        : status === "unread"
          ? "Completed: unread"
          : "Completed: read";
      continue;
