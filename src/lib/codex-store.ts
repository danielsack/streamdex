...
export class CodexStore {
  ...
  async refreshLiveComposer(force = false): Promise<void> {
    ...
    const readOnce = async (): Promise<void> => {
      ...
      try {
        ...
        // Failed observations also count toward the retry cadence.
  ...
  #open(): DatabaseSync {
    ...
    this.#threadColumns = new Set(
      (
        database.prepare("PRAGMA table_info(threads)").all() as unknown as {
          name: string;
        }[]
